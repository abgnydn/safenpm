import { AnalysisResult, ScriptWarning, riskLevel } from './analyzer'
import { TyposquatResult } from './typosquat'
import { DiffResult } from './diffing'
import { ThreatIntelResult } from './threatintel'
import { MaintainerInfo } from './maintainer'
import { LockfileAuditResult, LockfileIssue } from './lockfile'
import { ReputationSummary, PackageReputation } from './reputation'
...
export const logger = {
  ...
  blocked(pkgName: string, version: string, hook: string, reason: string = 'network') {
    console.log()
  ...
  allowed(pkgName: string, version: string) {
    console.log(`${GREEN}  ✓ clean${RESET}   ${pkgName}@${version}`)
  },

  skipped(pkgName: string, version: string) {
  ...
  typosquatResult(r: TyposquatResult) {
  ...
  threatIntelResult(r: ThreatIntelResult) {
    if (r.flagged) {
  ...
  maintainerResult(r: MaintainerInfo) {
  ...
  lockfileResult(result: LockfileAuditResult) {
    if (!result.exists) {
  ...
  reputationResult(summary: ReputationSummary) {
  ...
  dryRunItem(pkgName: string, version: string, hook: string, script: string, isAllowed: boolean) {
  ...
  summary(total: number, blocked: number, skippedCount: number = 0, warningCount: number = 0) {
    console.log()
    const parts: string[] = []
    if (blocked > 0) {
  ...
  interactivePrompt(pkgName: string, version: string, hook: string): string {
