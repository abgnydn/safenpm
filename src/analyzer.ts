...
export function analyzeScript(pkg: PackageScript): AnalysisResult {
...
export function analyzeAll(scripts: PackageScript[]): AnalysisResult[] {
  return scripts.map(analyzeScript)
}

export type RiskLevel = 'critical' | 'suspicious' | 'low' | 'clean'
