...
function levenshtein(a: string, b: string): number {
  const m = a.length
  const n = b.length
  // Flat row-major buffer: dp[i*W + j], width W = n + 1. Avoids the
  // nested-array hazards `noUncheckedIndexedAccess` flags inside an
  // arithmetic-bound inner loop.
  const W = n + 1
  ...
  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      const above = dp[(i - 1) * W + j]!
      const left = dp[i * W + (j - 1)]!
      const diag = dp[(i - 1) * W + (j - 1)]!
      dp[i * W + j] = a[i - 1] === b[j - 1] ? diag : 1 + Math.min(above, left, diag)
    }
  }
  return dp[m * W + n]!
}
...
function isTransposition(suspect: string, target: string): boolean {
  ...
  // diffs === 2 → diffPositions has exactly two entries.
  const a = diffPositions[0]!
  const b = diffPositions[1]!
