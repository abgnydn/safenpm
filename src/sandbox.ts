...
import os from 'os'
import fs from 'fs'
import { PackageScript, SandboxResult } from './types'
...
function buildSandboxProfile(pkgPath: string): string {
...
type SandboxBackend = 'sandbox-exec' | 'firejail' | 'windows-firewall' | 'wsl-firejail' | 'none'

function detectBackend(): SandboxBackend {
  const platform = os.platform()

  if (platform === 'darwin') {
    const r = spawnSync('which', ['sandbox-exec'], { encoding: 'utf8' })
    if (r.status === 0) return 'sandbox-exec'
  }

  if (platform === 'linux') {
    const r = spawnSync('which', ['firejail'], { encoding: 'utf8' })
    if (r.status === 0) return 'firejail'
  }

  if (platform === 'win32') {
    // Check 1: PowerShell with admin (can create firewall rules)
    if (isWindowsAdmin()) return 'windows-firewall'

    // Check 2: WSL with firejail available
    if (isWslFirejailAvailable()) return 'wsl-firejail'
  }
 ...
 */
function isWslFirejailAvailable(): boolean {
  try {
...
export function backendName(): string {
  const b = getBackend()
  if (b === 'sandbox-exec') return 'sandbox-exec (macOS)'
  if (b === 'firejail') return 'firejail (Linux)'
  if (b === 'windows-firewall') return 'Windows Firewall + ACLs (admin)'
  if (b === 'wsl-firejail') return 'firejail via WSL (Windows)'
  return 'none'
}
...
function runWithSandboxExec(pkg: PackageScript, strict: boolean): SandboxResult {
...
function runWithFirejail(pkg: PackageScript, strict: boolean): SandboxResult {
  const start = Date.now()
  ...
  const result = spawnSync('firejail', fjArgs, {
    cwd: pkg.path,
    env: cleanEnv(),
    timeout: 30_000,
    encoding: 'utf8',
 ...
 */
function runWithWindowsFirewall(pkg: PackageScript, strict: boolean): SandboxResult {
  const start = Date.now()
  const home = os.homedir()
 ...
 */
function runWindowsWithAclsOnly(
  pkg: PackageScript,
  strict: boolean,
 ...
 */
function runWithWslFirejail(pkg: PackageScript, strict: boolean): SandboxResult {
  const start = Date.now()
  ...
  if (strict) {
    fjArgs.push(
      `--blacklist=${wslHome}/.ssh`,
      `--blacklist=${wslHome}/.aws`,
      `--blacklist=${wslHome}/.gnupg`,
      `--blacklist=${wslHome}/.npmrc`,
      `--blacklist=${wslHome}/.netrc`,
      `--blacklist=${wslHome}/.docker`,
      `--blacklist=${wslHome}/.kube`,
      `--read-only=${wslHome}`,
    )
  }
  ...
  const result = spawnSync('wsl', fjArgs, {
    cwd: pkg.path,
    env: cleanEnv(),
    timeout: 30_000,
    encoding: 'utf8',
...
function runUnsandboxed(pkg: PackageScript): SandboxResult {
  const start = Date.now()

  // Use cmd.exe on Windows, sh elsewhere
  const shell = os.platform() === 'win32' ? 'cmd.exe' : 'sh'
  const shellArgs = os.platform() === 'win32' ? ['/c', pkg.script] : ['-c', pkg.script]
...
function classify(
  ...
  const timedOut = result.signal === 'SIGTERM' && durationMs >= 29_000
  ...
  let reason: SandboxResult['reason'] = 'error'
