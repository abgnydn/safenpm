...
exports.checkAllTyposquats = checkAllTyposquats;
 ...
 */
function isTransposition(suspect, target) {
 ...
 */
function hasCommonSubstitution(suspect, target) {
    const subs = [
        ['0', 'o'], ['1', 'l'], ['1', 'i'], ['rn', 'm'],
        ['vv', 'w'], ['-', ''], ['_', '-'], ['.', '-'],
    ];
    for (const [from, to] of subs) {
        if (suspect.replace(from, to) === target)
            return true;
        if (target.replace(from, to) === suspect)
            return true;
    }
    return false;
}
...
function checkAllTyposquats(names) {
    const results = [];
