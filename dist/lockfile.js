...
exports.significantLockfileIssues = significantLockfileIssues;
 ...
 */
function auditLockfile(projectDir) {
    const lockfilePath = path_1.default.join(projectDir, 'package-lock.json');
    ...
    let lockfile;
    try {
        lockfile = JSON.parse(fs_1.default.readFileSync(lockfilePath, 'utf8'));
    }
    catch {
        return {
            exists: true,
...
function detectFormat(lockfile) {
 ...
 */
function auditV1(lockfile, issues) {
...
function auditDependenciesRecursive(deps, issues, prefix, depth = 0, seen = new Set()) {
 ...
 */
function auditV2V3(lockfile, issues) {
 ...
 */
function checkResolvedUrl(url, shortName, fullName, issues) {
    // Git URLs
 ...
 */
function checkLockfileFreshness(projectDir, lockfilePath, issues) {
    try {
        const pkgJsonPath = path_1.default.join(projectDir, 'package.json');
        if (!fs_1.default.existsSync(pkgJsonPath))
            return;
        const pkgStat = fs_1.default.statSync(pkgJsonPath);
        const lockStat = fs_1.default.statSync(lockfilePath);
...
function countPackages(lockfile, format) {
    if (format === 'v1') {
...
function countDepsRecursive(deps, depth = 0) {
    if (depth >= MAX_AUDIT_DEPTH)
        return 0;
    let count = 0;
    for (const meta of Object.values(deps)) {
        count++;
        if (meta.dependencies)
            count += countDepsRecursive(meta.dependencies, depth + 1);
    }
    return count;
}
function calculateScore(issues, totalPackages) {
    if (totalPackages === 0)
        return 100;
 ...
 */
function significantLockfileIssues(result) {
