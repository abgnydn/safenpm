...
exports.snapshotPackage = snapshotPackage;
exports.diffPackage = diffPackage;
exports.diffAllPackages = diffAllPackages;
exports.snapshotAllPackages = snapshotAllPackages;
exports.formatDiffForTerminal = formatDiffForTerminal;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const os_1 = __importDefault(require("os"));
const CACHE_DIR = path_1.default.join(os_1.default.homedir(), '.safenpm', 'pkg-snapshots');
/**
 * Take a snapshot of a package for future diffing.
 */
function snapshotPackage(pkgDir, name, version) {
    try {
        ...
        fs_1.default.writeFileSync(file, JSON.stringify(snapshot), 'utf8');
    }
    catch { /* best effort */ }
}
/**
 ...
 */
function diffPackage(pkgDir, name, version) {
    const prev = loadSnapshot(name);
    const pkgJsonPath = path_1.default.join(pkgDir, 'package.json');
    let currentPkg = {};
    try {
        currentPkg = JSON.parse(fs_1.default.readFileSync(pkgJsonPath, 'utf8'));
    }
    catch { /* skip */ }
    ...
    const addedFiles = currentFiles.filter((f) => !prevFileSet.has(f));
    const removedFiles = prevFiles.filter((f) => !currFileSet.has(f));
    ...
    if (scriptDiffs.length > 0)
        parts.push(`${scriptDiffs.length} script change${scriptDiffs.length > 1 ? 's' : ''}`);
    if (addedDeps.length > 0)
        parts.push(`${addedDeps.length} new dep${addedDeps.length > 1 ? 's' : ''}`);
    if (removedDeps.length > 0)
        parts.push(`${removedDeps.length} removed dep${removedDeps.length > 1 ? 's' : ''}`);
    if (addedFiles.length > 0)
        parts.push(`${addedFiles.length} new file${addedFiles.length > 1 ? 's' : ''}`);
    return {
        name,
 ...
 */
function diffAllPackages(nodeModulesDir) {
    const diffs = [];
    ...
    for (const entry of entries) {
        ...
        if (entry.startsWith('@')) {
            try {
                ...
                for (const scopeEntry of fs_1.default.readdirSync(scopeDir)) {
                    const pkgDir = path_1.default.join(scopeDir, scopeEntry);
            ...
            const pkgDir = path_1.default.join(nodeModulesDir, entry);
            const version = readVersion(pkgDir);
            if (version) {
 ...
 */
function snapshotAllPackages(nodeModulesDir) {
    let count = 0;
    if (!fs_1.default.existsSync(nodeModulesDir))
        return count;
    const entries = fs_1.default.readdirSync(nodeModulesDir);
    for (const entry of entries) {
        ...
        if (entry.startsWith('@')) {
            try {
                for (const se of fs_1.default.readdirSync(path_1.default.join(nodeModulesDir, entry))) {
                    const pkgDir = path_1.default.join(nodeModulesDir, entry, se);
                    const ver = readVersion(pkgDir);
                    if (ver) {
                        snapshotPackage(pkgDir, `${entry}/${se}`, ver);
                        count++;
                    }
                }
            }
            ...
            const ver = readVersion(pkgDir);
            if (ver) {
                snapshotPackage(pkgDir, entry, ver);
                count++;
            }
        }
    }
...
function readVersion(pkgDir) {
    try {
        const p = path_1.default.join(pkgDir, 'package.json');
        if (!fs_1.default.existsSync(p))
            return null;
        return JSON.parse(fs_1.default.readFileSync(p, 'utf8')).version || null;
    }
    catch {
        return null;
    }
 ...
 */
function computeLineDiff(prev, curr) {
...
function listFiles(dir, maxDepth, currentDepth = 0) {
...
function formatDiffForTerminal(diff) {
    ...
    if (diff.scriptDiff) {
        ...
        if (sd.added) {
            ...
            for (const dl of sd.diffLines) {
                if (dl.type === 'remove')
                    lines.push(`  \x1b[31m- ${dl.content}\x1b[0m`);
                else if (dl.type === 'add')
                    lines.push(`  \x1b[32m+ ${dl.content}\x1b[0m`);
                else
    ...
    if (diff.fileDiff) {
        ...
        if (fd.added.length > 0) {
            ...
            for (const f of fd.added.slice(0, 5))
                lines.push(`    \x1b[32m+ ${f}\x1b[0m`);
            if (fd.added.length > 5)
                lines.push(`    \x1b[2m...and ${fd.added.length - 5} more\x1b[0m`);
        }
        if (fd.removed.length > 0) {
            lines.push(`  \x1b[31m- ${fd.removed.length} file${fd.removed.length > 1 ? 's' : ''} removed\x1b[0m`);
            for (const f of fd.removed.slice(0, 5))
